...
export default function ValuationWorkspace() {
  ...
  const onResult = useCallback((r: ProResult) => {
    setResult(r);
  ...
  const runValuation = useCallback(async (url: string, body: Record<string, unknown>) => {
    setLoading(true);
    setLastReq({ url, body });
    try {
  ...
  const revalue = useCallback(async () => {
  ...
  useEffect(() => {
    ...
    const pendingCode =
      typeof window !== "undefined"
        ? new URLSearchParams(window.location.search).get("code") ?? sessionStorage.getItem("tv1_pending_code")
        : sessionStorage.getItem("tv1_pending_code");
    ...
    <div className="min-h-screen">
      ...
      <main className="mx-auto max-w-5xl space-y-6 px-5 py-8">
        ...
        <section className={`card p-6 ${!assump ? "opacity-50" : ""}`}>
          ...
          {assump ? (
            ...
            <p className="mt-2 text-sm text-[color:var(--muted)]">
              Select a company above — its growth, margin and discount rate are derived automatically and become editable here.
            </p>
          )}
        </section>
        ...
        <section className={`card p-6 ${!result && !loading ? "opacity-50" : ""}`}>
          ...
          {!loading && !result && (
            <p className="text-sm text-[color:var(--muted)]">
              The triangulated range, the three approaches, the LLM analyst&apos;s note and the comparable set appear here.
            </p>
          )}
        </section>

        <footer className="pb-6 text-center text-[11px] text-[color:var(--muted)]">
          Indicative screening valuation aligned to ICAI/IBBI methodology — not a certified registered-valuer opinion.
        </footer>
      </main>
    </div>
  );
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3">
        ...
        <span className="text-xs text-[color:var(--muted)]">
          PDF auto-extraction plugs in from the dedicated extraction flow
        </span>
      </div>
    </div>
  );
