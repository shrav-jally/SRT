...
def build_db():
    ...
    # Intentionally leaving 396 companies with NULLs (Direct approach)
    ...
    print("4. Loading to SQLite (comp_v3_direct.db)...")
    conn = sqlite3.connect('database/comp_v3_direct.db')
    final_df.to_sql('comps', conn, if_exists='replace', index=False)
    conn.close()
    print("Done building comp_v3_direct.db")
