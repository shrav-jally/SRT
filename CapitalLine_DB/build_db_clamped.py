...
def build_db():
    ...
    print("3. Mapping to Schema and Recovering Data (Clamped)...")
    rename_map = {
    ...
    # CLAMPING LOGIC (Match Capitaline logic: negative valuation multiples become 0.0)
    merged.loc[merged['pe'] < 0, 'pe'] = 0.0
    merged.loc[merged['ev_ebitda'] < 0, 'ev_ebitda'] = 0.0
    merged.loc[merged['ev_revenue'] < 0, 'ev_revenue'] = 0.0
    merged.loc[merged['mktcap_sales'] < 0, 'mktcap_sales'] = 0.0
    
    merged['code'] = merged['code'].astype(int)
    final_cols = ['code', 'name', 'sector', 'industry', 'mode', 'year_end', 'revenue', 'ebitda', 'pat', 'net_worth', 'total_debt', 'net_debt', 'cash', 'market_cap', 'enterprise_value', 'pe', 'ev_ebitda', 'ev_revenue', 'mktcap_sales']
    ...
    print("4. Loading to SQLite (comp_v3_clamped.db)...")
    conn = sqlite3.connect('database/comp_v3_clamped.db')
    final_df.to_sql('comps', conn, if_exists='replace', index=False)
    conn.close()
    print("Done building comp_v3_clamped.db")
