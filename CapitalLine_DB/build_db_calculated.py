...
def build_db():
    ...
    print("3. Mapping to Schema and Recovering Data (Pure Math)...")
    rename_map = {
    ...
    # RECOVER MISSING MULTIPLES (Pure Math, retains negatives)
    ...
    print("4. Loading to SQLite (comp_v3_calculated.db)...")
    conn = sqlite3.connect('database/comp_v3_calculated.db')
    final_df.to_sql('comps', conn, if_exists='replace', index=False)
    conn.close()
    print("Done building comp_v3_calculated.db")
